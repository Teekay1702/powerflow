...
// Convert a system recommendation into editable line items.
function recommendationToLineItems(rec: SystemRecommendation): LineItem[] {
  const all: RecommendedProduct[] = [
    ...rec.products.panels,
    ...rec.products.batteries,
    ...rec.products.inverters,
    ...rec.products.accessories,
  ];
  return all.map((p) => ({
    id: `rec-${p.productId}`,
    itemType: p.itemType,
...
function NewQuotationContent() {
  ...
  // Customers are loaded through the SAME query as the Customers page so the
  // dropdown always matches what's shown there (and auto-refreshes).
  const { data: customersQuery } = useCustomers('', 1, 200);

  const [customerId, setCustomerId] = useState('');
  const [electricalLoadId, setElectricalLoadId] = useState(loadIdParam ?? '');
  ...
  const { data: recommendationData, isLoading: recLoading } = useSystemRecommendation(loadIdParam);
  const recommendation: SystemRecommendation | undefined = recommendationData?.data;

  // Pre-fill line items from the recommended system once it loads (and only if
  // the user hasn't already added/edited items).
  useEffect(() => {
    if (recommendation && items.length === 0) {
      const recItems = recommendationToLineItems(recommendation);
      if (recItems.length > 0) setItems(recItems);
    }
  ...
  const customers = customersQuery?.data ?? formData?.customers ?? [];
      ...
      {loadIdParam && (
        <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50/60 p-4">
          ...
          <div className="flex-1">
            {recLoading ? (
              <p className="text-sm text-slate-600">Loading recommended system…</p>
            ) : recommendation ? (
              <>
                <p className="text-sm font-semibold text-emerald-800">
                  Recommended minimum system loaded
                </p>
                <p className="text-xs text-emerald-700 mt-0.5">
                  {recommendation.sizing.solar.numberOfPanels} panels,{' '}
                  {recommendation.sizing.battery.numberOfBatteries} × battery,{' '}
                  {recommendation.sizing.inverter.totalInverterCapacity} VA inverter.
                  Adjust any quantity below to install more (or less) than recommended.
                </p>
              </>
            ) : (
              <p className="text-sm text-slate-600">
                Could not load a recommendation for this load profile. Add items manually.
              </p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={customerId} onValueChange={setCustomerId}>
                <SelectTrigger>
                  <SelectValue placeholder={customers.length ? 'Select a customer' : 'No customers available'} />
                </SelectTrigger>
                <SelectContent>
                  {customers.map((c: any) => (
              ...
              {customers.length === 0 && (
                <p className="text-xs text-amber-600">
                  No active customers found. <Link href="/dashboard/customers" className="underline">Add a customer first.</Link>
                </p>
              )}
              {customers.length > 0 && (
